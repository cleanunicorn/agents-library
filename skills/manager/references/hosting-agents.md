...
level closes only the ids it created and recorded — never by label, never
from a listing. A manager never closes its own workspace. A host with no
workspaces records `workspace: n/a` and what isolates the agents instead.
...
3. **Watch by rotating** `herdr agent wait <manager> --until blocked --until
   ...
   question: read it with `herdr agent read <manager>` and relay it.
...
3. **Start and drive each member through the agent commands:**
   ...
   same way. Never answer it for the user.
...
|------|------------------------------------------|----------------------------------|----------------------------|
| 1 | `agent wait` returns `blocked` or `idle`, and `status.md` reads `blocked` | `<run_dir>/questions.md`; the pane, for an approval prompt | `herdr agent prompt <manager> "<id>: <answer>"` |
...
- **A manager:** the super manager starts a replacement in the same workspace
  with the same run directory and launch prompt; it re-runs the gate before
  trusting Progress and reuses the recorded team sub-space id, or opens a new
  one and records both.
...
1. The agent invokes the skill by name.
