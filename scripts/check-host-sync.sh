#!/usr/bin/env bash
# Verify every host mirror (.opencode/, .kilo/) is in sync with agents/ and
# skills/. Run this before merging when agents/ or skills/ changes.
...
check_host() {
  ...
  for f in agents/*.md; do
    [ -f "$f" ] || continue
    name=$(basename "$f" .md)
    check_link "$f" "$dir/agents/${name}.md"
  done

  for d in skills/*/; do
    [ -f "${d}SKILL.md" ] || continue
    name=$(basename "$d")
    check_link "${d%/}" "$dir/skills/${name}"
  done

  for link in "$dir"/agents/*.md; do
    [ -L "$link" ] || continue
    name=$(basename "$link" .md)
    [ -f "agents/${name}.md" ] || { echo "STALE: $link — no source agent agents/${name}.md"; status=1; host_status=1; }
  done
