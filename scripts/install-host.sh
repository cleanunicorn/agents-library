#!/usr/bin/env bash
# Install the library's agents and skills into a host's discovery paths.
# Run `install-host.sh --help` for full usage. The per-host wrappers
# install-opencode.sh and install-kilo.sh pass the host automatically.
...
Options:
  ...
  --project         Install to ./.<host>/ in the current directory
...
the host's global config directory using absolute symlinks, so `git pull` in
this repo updates every linked install.  Use --project for a self-contained
copy in a specific project, or --copy anywhere files should not depend on
this clone.
...
host="${1:-}"
if [ -z "$host" ]; then
  echo "FATAL: missing host — expected opencode or kilo" >&2
  exit 1
fi
shift
...
case "$host" in
  ...
  *) echo "FATAL: unknown host: $host — expected opencode or kilo" >&2; exit 1 ;;
esac
...
case "$scope" in
  global)  dest_root="$global_root" ;;
  project) dest_root="$project_root" ;;
esac
