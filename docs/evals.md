...
|-------|---------|----------------------|
| plan-feature | layered task app with behavioral tests, a missing-harness variant, and an existing plan | plans cite real integration points and observable tests; report-only leaves the baseline intact; requested plan updates preserve unrelated content |
